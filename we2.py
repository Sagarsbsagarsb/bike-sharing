...
api_data = pd.read_csv(io.StringIO(api_data_str), sep=",")
maintenance_data = pd.read_csv(io.StringIO(maintenance_str), sep=",")

# Convert timestamp
api_data["timestamp"] = pd.to_datetime(api_data["timestamp"], errors="coerce")
...
# Encode status
le = LabelEncoder()
api_data["status_encoded"] = le.fit_transform(api_data["status"])

# Define features & target
X = api_data[["temperature", "vibration", "pressure", "anomaly_score"]]
y = api_data["status_encoded"]
...
page = st.sidebar.radio("Go to", ["Overview", "Fault Simulator", "Maintenance Logs"])
...
if page == "Overview":
    ...
    with col1:
        ...
        st.metric("Total Equipment", api_data["equipment_id"].nunique())
        st.metric("Open Maintenance Tickets", maintenance_data[maintenance_data["status"].str.contains("Open|In Progress")].shape[0])

    with col2:
        st.write("📊 Status Distribution")
        fig, ax = plt.subplots()
        api_data["status"].value_counts().plot(kind="bar", ax=ax, color=["green", "orange", "red"])
        st.pyplot(fig)

    st.subheader("Recent Sensor Data")
    st.dataframe(api_data.tail(10))

# -----------------------
# Fault Simulator
# -----------------------
elif page == "Fault Simulator":
    st.subheader("⚡ Prediction Simulator: e-Bike Component Faults")

    vibration = st.slider("Vibration (g-force)", 0.0, 5.0, 1.5, 0.1)
    temperature = st.slider("Motor Temp (°C)", 40, 120, 75, 1)
    pressure = st.slider("Pressure (bar)", 0.5, 10.0, 5.0, 0.1)
    anomaly_score = st.slider("Anomaly Score", 0.0, 1.0, 0.5, 0.01)

    # Make prediction
    input_data = np.array([[temperature, vibration, pressure, anomaly_score]])
    pred = model.predict(input_data)[0]
    pred_label = le.inverse_transform([pred])[0]

    if pred_label == "OK":
        st.success("✅ Fault Risk: Low - Systems operating normally")
    elif pred_label == "Warning":
        st.warning("⚠️ Fault Risk: Medium - Monitor system closely")
    else:
        st.error("🚨 Fault Risk: High - Critical maintenance required!")
...
# -----------------------
    ...
    st.write("📅 Open / In Progress Issues")
    st.dataframe(maintenance_data[maintenance_data["status"].str.contains("Open|In Progress")])
